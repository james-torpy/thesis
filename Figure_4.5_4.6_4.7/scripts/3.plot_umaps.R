...
PC <- args[8]
garnett_slot <- args[9]
remove_outliers <- args[10]
outlier_sd_multiplier <- as.numeric(args[11])
...
print(paste0("Resolution for epithelial cell plots = ", res))
print(paste0("PC for epithelial cell plots = ", PC))
...
if (
  ...
  # only keep non-outlying cells in metadata:
  seurat_epi <- subset(
    seurat_epi,
    cells = epithelial_metadata$cell_ids[
      epithelial_metadata$exp_id_pre_normal_call != "Epithelial_outlier"
    ]
  )

  # assign idents:
  Idents(seurat_epi) <- gsub(
    "_", " ", 
    epithelial_metadata$exp_id_pre_normal_call[
      epithelial_metadata$exp_id_pre_normal_call != "Epithelial_outlier"
    ]
  )
  levels(Idents(seurat_epi)) <- naturalsort(levels(Idents(seurat_epi)))
}
...
if (
  ...
  epi_umap <- DimPlot(
    ...
    reduction = paste0("UMAP", PC),
    label = F
    #order = as.character(levels(Idents(seurat_epi)))
  ) + theme(
...
temp_seurat_epi <- remove_UMAP_outliers(seurat_epi, PC)
...
if (met) {
  ...
  # update Idents to post-normal call:
  epithelial_metadata_no_normal <- epithelial_metadata[
    epithelial_metadata$cell_ids %in% names(Idents(seurat_epi_no_normal)),
  ]
  Idents(seurat_epi_no_normal) <- epithelial_metadata_no_normal$exp_id_post_normal_call
  seurat_epi_no_normal <- subset(
    seurat_epi_no_normal,
    cells = epithelial_metadata_no_normal$cell_ids[
      epithelial_metadata_no_normal$exp_id_post_normal_call != "Epithelial_outlier"
    ]
  )

  # remove outliers:
  seurat_epi_no_normal_filtered <- remove_UMAP_outliers(seurat_epi_no_normal, PC)[[2]]
  Idents(seurat_epi_no_normal_filtered) <- gsub(
    "_", " ", 
    Idents(seurat_epi_no_normal_filtered)
  )
  levels(Idents(seurat_epi_no_normal_filtered)) <- naturalsort(levels(Idents(seurat_epi_no_normal_filtered)))
