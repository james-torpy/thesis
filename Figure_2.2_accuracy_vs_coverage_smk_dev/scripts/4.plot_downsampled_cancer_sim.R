...
downsample_proportions <- strsplit(args[3], split = "_")
CNV_type <- args[4]
simulation_number <- args[5]
...
# load functions:
summarySE <- dget(paste0(func_dir, "prepare_infercnv_metadata.R"))
...
for (i in 1:length(simulation_numbers)) {
  ...
  metrics_list <- lapply(downsample_list, function(x) {
    ...
    print(paste0("Fetching metrics for simulation ", 
      ...
      in_dir <- paste0(in_path, "/", simulation_number, "/", 
    x, "_downsampling/", analysis_mode, 
    "_mode/Rdata/")
...
cols <- c("#7CBA61", "#9DC9DC", "#B488B4", "#F6DC15", "#C02456")

p <- ggplot(plot_metrics, aes(x = Proportion, y = Score)) 
p <- p + geom_line(aes(color = Metric))
p <- p + scale_color_manual(values = cols[1:4])
p <- p + scale_x_continuous(
  breaks = c(0, 4, seq(10, 100, 10))
)
