...
if (gap_or_CNV == "CNV") {
  ...
  feature_indices <- CNV_data[[1]]$CNV_record
  CNV_indices <- CNV_data[[1]]$CNV_record

} else {

  if (CNV_type == "gain") {
    if (CNV_data[[1]]$extended_gap_record$multiplier[1] == 3) {
      feature_indices <- CNV_data[[1]]$extended_gap_record
      CNV_indices <- CNV_data[[1]]$CNV_record
    } else if (CNV_data[[1]]$extended_gap_record$multiplier[1] == 0) {
      feature_indices <- CNV_data[[2]]$extended_gap_record
      CNV_indices <- CNV_data[[2]]$CNV_record
    }
  } else if (CNV_type == "loss") {
    if (CNV_data[[1]]$extended_gap_record$multiplier[1] == 0) {
      feature_indices <- CNV_data[[1]]$extended_gap_record
      CNV_indices <- CNV_data[[1]]$CNV_record
    } else if (CNV_data[[1]]$extended_gap_record$multiplier[1] == 3) {
      feature_indices <- CNV_data[[2]]$extended_gap_record
      CNV_indices <- CNV_data[[2]]$CNV_record
    }
  }
...
log_modified_fold_change_df <- CNV_data[[1]]$log_modified_fold_change_df
...
if (gap_or_CNV == "CNV") {
  ...
  if (
    ...
    feature_indices$call <- "gap not called"

    # annotate all gain ranges within neutral_signal_ranges as "gap called":
    feature_indices$call[
      feature_indices$mean_signal >= neutral_signal_range[1] &
      feature_indices$mean_signal <= neutral_signal_range[2]
    ] <- "gap called"
    ...
    CNV_calls <- read.table(
      paste0(table_dir, "gap_calls.txt"),
      sep = "\t",
      header = T,
      as.is = T
    )
    CNV_indices <- readRDS(paste0(Robject_dir, "3.final_gap_indices.Rdata"))
...
if (gap_or_CNV == "CNV") {
  ...
  levels(call_annotation_vector) <- c("CNV called", "CNV not called", "non-CNV")
  ...
  if (gap_or_CNV == "CNV") {
    ...
    just = "right"))
      grid.text(
        paste0(
          "Correct gain calls = ", 
          length(which(feature_calls$call == "CNV called" & feature_calls$type == "gain"))
        ), gp=gpar(fontsize=16)
      )
    popViewport()
    pushViewport(viewport(x=x_coord + 0.075, y=0.83, width = 0.1, height = 0.1, 
      ...
      grid.text(
        paste0(
          "Correct loss calls = ", 
          length(which(feature_calls$call == "CNV called" & feature_calls$type == "loss"))
        ), gp=gpar(fontsize=16)
      )
    popViewport()
    pushViewport(viewport(x=x_coord + 0.07, y=0.8, width = 0.1, height = 0.1, 
      ...
      grid.text(
        paste0(
          ...
          length(which(feature_calls$call == "CNV not called" & feature_calls$type == "gain"))
        ), gp=gpar(fontsize=16)
      )
    popViewport() 
    pushViewport(viewport(x=x_coord + 0.073, y=0.77, width = 0.1, height = 0.1, 
      ...
      grid.text(
        paste0(
          ...
          length(which(feature_calls$call == "CNV not called" & feature_calls$type == "loss"))
        ), gp=gpar(fontsize=16)
      )
    popViewport() 
    ...
    just = "right"))
      grid.text(
        paste0(
          "Correct calls = ", 
          length(which(feature_calls$call == "gap called"))
        ), gp=gpar(fontsize=16)
      )
    popViewport()
    pushViewport(viewport(x=x_coord + 0.075, y=0.83, width = 0.1, height = 0.1, 
      ...
      grid.text(
        paste0(
          "Incorrect calls = ", 
          length(which(feature_calls$call == "gap not called"))
        ), gp=gpar(fontsize=16)
      )
    popViewport()
