...
infercnv_df <- data.frame(score = infercnv_output)
p <- ggplot(infercnv_df, aes(score)) + 
  geom_density() + 
  geom_vline(xintercept = 0.915, colour = "red") + 
  geom_vline(xintercept = 1.085, colour = "red")
...
rough_neutral_df <- data.frame(
  score = infercnv_df[infercnv_df$score > 0.915 & infercnv_df$score < 1.085,]
)
...
################################################################################
### 4. Calculate standard deviation of neutral signal and denoise values within
# 1, 1.5 and 2 standard deviations from mean ###
################################################################################

mean_signal <- mean(infercnv_df$score)
signal_sd <- sd(infercnv_df$score)

# make infercnv scores that fall within n std devs from mean equal to mean
d_vals <- c(1, 1.5, 2)
