...
project_name <- "thesis"
subproject_name <- "Figure_1.8_accuracy_vs_coverage"
sample_name <- args[1]
numcores <- as.numeric(args[2])
subpops <- args[3]
simulation_number <- args[4]
analysis_mode <- args[5]
p_val <- as.numeric(args[6])
downsample <- as.logical(args[7])
downsample_proportion <- as.character(args[8])
...
print(paste0("Subpops? ", as.character(subset_data)))
...
in_dir <- paste0(results_dir, "infercnv/", sample_name, "/", subpops,
