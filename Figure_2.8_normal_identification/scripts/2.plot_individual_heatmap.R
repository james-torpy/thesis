...
################################################################################
### 3. Add CNA and cancer correlation metadata ###
################################################################################
...
################################################################################
### 4. Remove clusters without enough epithelial cells ###
################################################################################
...
if (nrow(epithelial_metadata) < 20) {
  epithelial_heatmap <- readRDS(
    paste0(Robject_dir, 
    "3a.epithelial_heatmap_with_cell_type_QC_CNA_and_correlation_values.Rdata")
  )
...
################################################################################
### 5a. Call normals and add to epithelial_metadata ###
################################################################################
...
#    ######
#    # plot CNA values and correlation values vs epithelial markers
#    # indicating normal or cancer:
#    if (!exists("seurat_10X")) {
#      seurat_10X <- readRDS(
#        paste0(seurat_dir, "03_seurat_object_processed.Rdata")
#      )
...
#    ]
#    
#    # create df of count values for each marker:
#    for (m in 1:length(epithelial_markers)) {
#      print(paste0("Adding ", epithelial_markers[m], 
#        " to CNA value vs epithelial marker plot..."))
#      if (m==1) {
#        gene_expression_df <- data.frame(cell_ids = epithelial_metadata$cell_ids,
#          expression = as.numeric(count_df[epithelial_markers[m],]), 
#          marker = epithelial_markers[m])
#      } else {
#        gene_expression_df <- rbind(
#          gene_expression_df,
#          data.frame(cell_ids = epithelial_metadata$cell_ids,
#          expression = as.numeric(count_df[epithelial_markers[m],]), 
#          marker = epithelial_markers[m])
#        )
#      }
...
#    
#    # add CNA values:
#    CNA_df <- subset(epithelial_metadata, select = c(cell_ids, CNA_value))
#    genes_vs_CNA <- merge(gene_expression_df, CNA_df, 
#      by="cell_ids")
#
#    # log10 expression values:
#    genes_vs_CNA$expression <- log10(genes_vs_CNA$expression)
#      
#    p <- ggplot(genes_vs_CNA, aes(CNA_value, expression, colour = marker))
#    p <- p + geom_smooth(method=lm)
#    p <- p + geom_point()
#    
#    print(paste0("Printing CNA value vs epithelial marker plot..."))
#    pdf(paste0(plot_dir, "epithelial_markers_vs_CNA_values.pdf"))
#      print(p)
#    dev.off()
#    ######
    ...
    ################################################################################
    ### 5b. Call normals and add to epithelial_metadata ###
    ################################################################################
...
################################################################################
### 11. Create all cell and garnett tSNE and reclustered epithelial UMAP ###
################################################################################
...
customize_plots <- function(splot, type="tSNE", garnett = F) {
