...
if (downsample) {
  out_path <- paste0(results_dir, "cancer_simulation/", sample_name, "_cancer_sim/")
...
if (downsample) {
  chromosome_CNV_intervals <- data.frame(
    lengths =    c(400, 400, 300, 300, 200,  200,    #1
      ...
      #150, 150,  125,  125,    100, 100, 75, 75,    #3
      #150, 125, 100, 75, 300,                       #4
      1600, 200, 200, 200, 200, 200,                 #5
      200, 200, 200, 200, 200),                      #6
    multiplier = c(3,   0,    2,  0.5,  1.5,  0.5,   #1
      1,   3,     0,    3,  0,   3,   0,  3,  0,     #2
      #2,   0.5,   2,    0.5,   2,   0.5,  2,  0.5,  #3
      #1.5, 1.5, 1.5, 1.5, 1,                        #4
      1,    3,   3,   3,    3,  3,                   #5
      0,   0,   0,    0,  0),                        #6
...
if (!file.exists(paste0(Robject_dir, "/1.original_count_df.Rdata"))) {
  # load seurat object:
  ...
  if (downsample) {
    ...
    # create count value vs cancer correlation quad plot:
    print("Determining scaled total count values...")
    # scale infercnv values to -1:1, square values and take the mean:
    scaled_df <- as.data.frame(rescale(as.matrix(epithelial_df), c(-1,1)))
    count_values <- apply(scaled_df, 2, function(y) {
      return(mean(y^2))
    })
    count_cor_df <- data.frame(
      row.names = names(count_values),
      count_value = count_values
    )
  
    # determine correlation with cells with top 5% expression:
    print(paste0(
      "Determining correlation with top 5% cancer values and adding to epithelial ", 
      "metadata df..."
    ))
    # determine cells with top 5% expression:
    count_order <- order(count_cor_df$count_value, decreasing=T)
    ordered_count_values  <- data.frame(
      row.names = rownames(count_cor_df)[count_order],
      count_value = count_cor_df[count_order,]
    )
    top_expression <- head(ordered_count_values, nrow(ordered_count_values)*0.05)
    
    # find average genome-wide expression for top cells:
    top_expression_average <- apply(epithelial_df[,rownames(top_expression)], 1, mean)
    # find correlations of each cell's CNVs with top_expression_average:
    top_expression_correlations <- apply(epithelial_df, 2, function(x) {
      if (length(unique(as.numeric(x))) == 1) {
        cor_result <- data.frame(cor.estimate="no_expression", 
          cor.p.value="no_expression")
      } else {
        cor <- cor.test(as.numeric(x), top_expression_average, method = "kendall")
        cor_result <- data.frame(cor$estimate, cor$p.value)
      }
      return(cor_result)
    })
    correlation_df <- do.call("rbind", top_expression_correlations)
  
    # add to epithelial_metadata:
    count_cor_df <- cbind(count_cor_df, correlation_df)
    print(paste0(
      "Are count_cor_df rownames in the same order as epithelial_heatmap?? ",
      identical(rownames(epithelial_heatmap), rownames(count_cor_df))
    ))
...
#  )
#  temp_df$chromosome <- rownames(chromosome_CNV_splits_2)[i]
#
#  CNV_indices <- rbind(CNV_indices, temp_df)
#}
...
system(paste0("cp ", metadata_dir, "metadata.txt ", out_dir, "input_files/"))
...
if (downsample) {
  ...
  print("Summary of counts before downsampling:")
  print(summary(apply(new_counts, 2, mean)))
  
  for (proportion in rev(seq(0.1, 0.9, 0.1))) {
  
    print(paste0("Downsampling counts by ", proportion, "..."))
    downsampled_counts <- downsampleMatrix(new_counts, proportion)
    print("Summary of downsampled counts:")
    print(summary(apply(downsampled_counts, 2, mean)))
  
    downsample_dir <- paste0(out_dir, proportion, "_downsampling/input_files/")
    system(paste0("mkdir -p ", downsample_dir))
    write.table(new_counts, paste0(downsample_dir, "input_matrix.txt"), quote=F,
      sep="\t", col.names=T, row.names=T)
  
    system(paste0("cp ", out_dir, "metadata.txt ", downsample_dir))
