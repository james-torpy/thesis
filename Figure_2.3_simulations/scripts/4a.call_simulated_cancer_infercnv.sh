...
for downsample_proportion in ${downsample_props[9]}
	...
	if [ "$downsample_proportion" = "1.0" ]
      ...
      qsub -wd $log_dir -pe smp $ncores -N ds.$downsample_proportion.$sample_name -b y -j y -V -P TumourProgression \
        ...
        CMD BATCH  --no-save '--args $sample_name $ncores $subset_data $include_t_cells $analysis_mode $downsample_proportion' $script_dir/4b.simulated_cancer_infercnv.R"
