...
normal_content <- as.numeric(args[9])
print(paste0("Proportion of normal cells = ", normal_content))
...
sim_name <- paste0("normals_", normal_content, "_", sim_name)
...
if (file.exists(paste0(table_dir, "random_seed_record.txt"))) {
  ...
  seed_record <- data.frame(
    row.names = c(
      "CNV_no", "start_position", "length", "multiplier", "normal_cell_indices"
    ),
    seed = sample(1:999, 5)
  )
  write.table(
...
if (
  ...
  # save summaries of filtered and non-filtered datasets:
  matrix_summaries <- t(
    data.frame(
      unfiltered_nUMI = as.vector(summary(apply(epithelial_df, 2, sum))),
      unfiltered_nGene = as.vector(summary(apply(
        epithelial_df, 2, function(x) length(x[x!=0])
      ))),
      filtered_nUMI = as.vector(summary(apply(filtered_epithelial_df, 2, sum))),
      filtered_nGene = as.vector(summary(apply(
        filtered_epithelial_df, 2, function(x) length(x[x!=0])
      )))
    )
  )
  colnames(matrix_summaries) <- c(
    "Min.", "1st Qu.",  "Median", "Mean", "3rd Qu.", "Max."
  )
  write.table(
    matrix_summaries, 
    paste0(table_dir, "filtered_vs_non_filtered_stats.txt"), 
    quote=F,
    sep="\t", 
    col.names=T, 
    row.names=T
...
if (sim_name != "filtered_normal") {
  ...
  # stash normal cells so they are not modified:
  if (normal_content > 0) {

    set.seed(seed_record["normal_cell_indices",])
    normal_indices <- sample(
      1:ncol(filtered_epithelial_df),
      floor(ncol(filtered_epithelial_df)*normal_content)
    )

    print(
      paste0("Removing ", floor(ncol(filtered_epithelial_df)*normal_content), 
        " cells from cancer simulation...")
    )

    normal_df <- filtered_epithelial_df[,normal_indices]
    cancer_df <- filtered_epithelial_df[,-normal_indices]

    print(
      paste0(
        ncol(filtered_epithelial_df), " in simulation before removal, ",
        ncol(cancer_df), " in simulation after removal. "
      )
    )

  } else {
    cancer_df <- filtered_epithelial_df
  }

  # determine total gene length of genome to ensure total CNV length < 0.4 x 
  # genome length to comply with InferCNV assumptions:
  genome_length <- nrow(cancer_df)
  ...
  if (
    ...
    for (i in 1:CNV_no) {
      ...
      if ( total_CNV_length < (genome_length*0.4) ) {
        ...
        repeat {
          ...
          if (end_position < nrow(cancer_df)) {
            if (exists("CNV_record")) {
        ...
        # add CNV to modified_df if exists, or original cancer_df if not:
        if (!exists("modified_df")) {
          modified_df <- cancer_df
        }
        ...
        average_original_counts <- apply(
          cancer_df[CNV_record$start[i]:CNV_record$end[i],], 1, mean
        )
    ...
    if (normal_content > 0) {
      saveRDS(normal_df, paste0(Robject_dir, "/2d.pre_noise_simulated_normal_df.Rdata"))
    }
    ...
    if (normal_content > 0) {
      normal_df <- readRDS(paste0(Robject_dir, "/2c.CNV_record.Rdata"))
    }
  ...
  # bind normal and modified dfs together:
  print("modified_df dimensions before adding normals:")
  print(dim(modified_df))
  combined_df <- cbind(
    normal_df,
    modified_df
  )
  print("combined_df dimensions after adding normals:")
  ...
  print("new_counts dimensions after adding non epithelial cells: ")
  ...
  for (i in 1:nrow(final_CNV_record)) {
    ...
    if (final_CNV_record$start[i] != final_CNV_record$end[i]) {
      average_original_counts <- apply(
        cancer_df[final_CNV_record$start[i]:final_CNV_record$end[i],], 1, mean)
    } else {
      average_original_counts <- mean(as.numeric(cancer_df[final_CNV_record$start[i],]))
    }
