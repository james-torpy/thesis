...
if (!file.exists(paste0(Robject_dir, "initial_combined_heatmap.Rdata")) | 
  ...
  ################################################################################
  ### 2. Add subtype and sample columns to group_heatmap_metadata ###
  ################################################################################
  ...
  saveRDS(group_heatmap_df, paste0(Robject_dir, "initial_group_heatmap_df.Rdata"))
  saveRDS(
  ...
  group_heatmap_df <- readRDS(paste0(Robject_dir, "initial_group_heatmap_df.Rdata"))
...
# order group_heatmap_df the same way as group_heatmap_metadata:
group_heatmap_df <- group_heatmap_df[rownames(group_heatmap_metadata),]
...
print(paste0("Are sample_annotation_df and group_heatmap_df rownames identical? ",
  identical(rownames(group_heatmap_df), rownames(sample_annotation_df))))
# determine colours and create annotation:
cluster_number <- length(unique(sample_annotation_df$sample))
cluster_cols <- rev(col_palette)[1:cluster_number]
...
#CNV_genes_annotation <- create_CNV_genes_annotation(group_heatmap_df, CNV_genes)
...
if (!file.exists(paste0(Robject_dir, "final_group_heatmap.Rdata"))) {
  ...
  rescaled_heatmap_df <- rescaled_heatmap_df[rownames(group_heatmap_metadata),]
  print(paste0("Are rescaled_heatmap_df and group_heatmap_metadata rownames identical? ",
    identical(rownames(rescaled_heatmap_df), rownames(group_heatmap_metadata))))
...
longest_cluster_name <- max(nchar(unique(as.character(group_heatmap_metadata$subtype))))
...
grid.newpage()
  ...
  popViewport()
  pushViewport(viewport(x=x_coord + 0.899, y=0.24, width = 0.1, height = 0.1, just = "bottom"))
    grid.text("CNA", rot=65, gp=gpar(fontsize=20))
  popViewport()
  pushViewport(viewport(x=x_coord + 0.919, y=0.24, width = 0.1, height = 0.1, just = "bottom"))
