...
loose_CNV_only <- lapply(loose_CNV_only, add_chromosome_indices, chr_positions)
...
if (!file.exists(
  ...
  loose_subpop_gr <- lapply(loose_CNV_only, function(x) {
    return(
      GRanges(
        seqnames = Rle(x$chr),
        ranges = IRanges(
  ...
  for (s in 1:length(subpop_CNV_gr)) {
    for (t in 1:length(loose_subpop_gr)) {

      gain_CNV_s <- subpop_CNV_gr[[s]][subpop_CNV_gr[[s]]$call == "gain"]
      gain_CNV_t <- subpop_CNV_gr[[t]][subpop_CNV_gr[[t]]$call == "gain"]
      loss_CNV_s <- subpop_CNV_gr[[s]][subpop_CNV_gr[[s]]$call == "loss"]
      loss_CNV_t <- subpop_CNV_gr[[t]][subpop_CNV_gr[[t]]$call == "loss"]

      gain_loose_s <- loose_subpop_gr[[s]][loose_subpop_gr[[s]]$call == "gain"]
      gain_loose_t <- loose_subpop_gr[[t]][loose_subpop_gr[[t]]$call == "gain"]
      loss_loose_s <- loose_subpop_gr[[s]][loose_subpop_gr[[s]]$call == "loss"]
      loss_loose_t <- loose_subpop_gr[[t]][loose_subpop_gr[[t]]$call == "loss"]

      if (s != t) {

        suppressWarnings(
          gain_olaps1 <- findOverlaps(gain_CNV_s, gain_loose_t)
        )
        suppressWarnings(
          gain_olaps2 <- findOverlaps(gain_CNV_t, gain_loose_s)
        )
        suppressWarnings(
          loss_olaps1 <- findOverlaps(loss_CNV_s, loss_loose_t)
        )
        suppressWarnings(
          loss_olaps2 <- findOverlaps(loss_CNV_t, loss_loose_s)
        )
  
        # if queryHits contains all query entries, merge:
        if (
          (is.sequential(unique(queryHits(gain_olaps1))) & 
          length(unique(queryHits(gain_olaps1))) == length(gain_CNV_s) & 
          is.sequential(unique(queryHits(loss_olaps1))) & 
          length(unique(queryHits(loss_olaps1))) == length(gain_CNV_s)) & 

          (is.sequential(unique(queryHits(gain_olaps2))) & 
          length(unique(queryHits(gain_olaps2))) == length(gain_CNV_t) & 
          is.sequential(unique(queryHits(loss_olaps2))) & 
          length(unique(queryHits(loss_olaps2))) == length(gain_CNV_t))
        ) {

            print(paste0("Merging ", names(subpop_CNV_gr)[s], " and ", 
            names(loose_subpop_gr)[t]))
  
            epithelial_metadata$revised_subcluster_id[
              epithelial_metadata$revised_subcluster_id == names(loose_subpop_gr)[t]
            ] <- names(subpop_CNV_gr)[s]
    
            subpop_CNV_gr[[s]] <- c(subpop_CNV_gr[[s]], subpop_CNV_gr[[t]])
            loose_subpop_gr[[s]] <- c(loose_subpop_gr[[s]], loose_subpop_gr[[t]])
  ...
  # remove any subpopulations secondary cluster ID:
...
levels(subcluster_annot_df$revised_subcluster_id) <- paste0(
  "CNV ", 
  1:length(unique(subcluster_annot_df$revised_subcluster_id))
)
