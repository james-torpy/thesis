...
min_CNV_proportion <- as.numeric(args[6])
...
print("Minimum proportion of signal to call CNV = ")
...
print(paste0("CNV type = ", as.character(CNV_type)))
...
# accuracy annotation legend functions:
...
true_pos_neg_false_pos_neg_wrong_legend <- function() {
  # plot legend text:
  pushViewport(viewport(x = 0.24, y = 0.96, 
                        width = unit(0.1, "cm"), height = unit(0.1, "cm")))
    #grid.rect()
    grid.text("true positive", gp=gpar(fontsize=18), just = c("left"))
  popViewport()
  pushViewport(viewport(x = 0.24, y = 0.76, 
                        width = unit(0.1, "cm"), height = unit(0.1, "cm")))
    #grid.rect()
    grid.text("true negative", gp=gpar(fontsize=18), just = c("left"))
  popViewport()
  pushViewport(viewport(x = 0.24, y = 0.56, 
                        width = unit(0.1, "cm"), height = unit(0.1, "cm")))
    #grid.rect()
    grid.text("false positive", gp=gpar(fontsize=18), just = c("left"))
  popViewport()
  pushViewport(viewport(x = 0.24, y = 0.36, 
                        width = unit(0.1, "cm"), height = unit(0.1, "cm")))
    #grid.rect()
    grid.text("false negative", gp=gpar(fontsize=18), just = c("left"))
  popViewport()
  pushViewport(viewport(x = 0.24, y = 0.16, 
                        width = unit(0.1, "cm"), height = unit(0.1, "cm")))
    #grid.rect()
    grid.text("wrong call", gp=gpar(fontsize=18), just = c("left"))
  popViewport()
  
  # plot legend squares:
  pushViewport(viewport(x = 0, y = 0.9, 
                        width = unit(1, "cm"), height = unit(0.5, "cm"), 
                        just = c("left")))
    grid.rect(width = unit(5, "mm"), height = unit(5, "mm"),
            just = c("left", "bottom"), gp=gpar(col = "#430F82", fill = "#430F82"))
  popViewport()
  pushViewport(viewport(x = 0, y = 0.7, 
                        width = unit(1, "cm"), height = unit(0.5, "cm"), 
                        just = c("left")))
    grid.rect(width = unit(5, "mm"), height = unit(5, "mm"),
              just = c("left", "bottom"), gp=gpar(col = "#B488B4", fill = "#B488B4"))
...
if (!file.exists(paste0(plot_dir, "sim_CNV_annotation_plot.png"))) {
  png(paste0(plot_dir, "sim_CNV_annotation_plot.png"),
    width = 20, height = 8, res = 300, units = "in")
...
if (!file.exists(
  ...
  print(paste0("Number of true positive genes = ", accuracy_metrics["true_positive",]))
  print(paste0("Number of true negative genes = ", accuracy_metrics["true_negative",]))
  print(paste0("Number of false positive genes = ", accuracy_metrics["false_positive",]))
  print(paste0("Number of false negative genes = ", accuracy_metrics["false_negative",]))
  print(paste0("Number of wrong calls = ", accuracy_metrics["wrong_call",]))
...
png(
  paste0(plot_dir, "infercnv_plot_with_marked_CNVs.png"), 
  height = 13, width = 23, res = 300, units = "in"
)   
  pushViewport(viewport(x = 0.147, y = 0.3, width = 0.805, height = 0.67, 
      just = c("left", "bottom")))
...
################################################################################
### 12. Annotate true/false positives/negatives ###
################################################################################

# create annotation of true/false positives/negatives:
accuracy_annotation_vector <- factor(
  CNV_accuracy_df$accuracy_call,
  levels = c(
...
accuracy_cols <- structure(
  c("#430F82", "#F6DC15",
    "#B488B4", "#7CBA61", 
    "#C02456"),
  names = levels(accuracy_annotation_vector)
)
accuracy_heatmap <- Heatmap(
  t(matrix(accuracy_annotation_vector)),
  name = "accuracy_heatmap",
  col = accuracy_cols,
...
#################################################################################
#### 13. Plot average signal vs accuracy annotation ###
#################################################################################
...
# expand CNVs less than 40 genes long:
if (ncol(plot_object) > 4000) {
...
# calculate midpoints for CNV_indices and label those to be included:
CNV_indices$midpoints = CNV_indices$start + floor(CNV_indices$length/2)
CNV_indices$ticks = "not_include"
CNV_indices$ticks[CNV_indices$multiplier != 1] = "include"
...
png(
  ...
  grid.newpage()
    pushViewport(viewport(x = 0.52, y = 0.47, width = 0.93, height = 0.942))
      grid.draw(signal_plot)
    popViewport()
    pushViewport(viewport(x = 0.52, y = 0.231, width = 0.785, height = 0.13))
      grid.draw(accuracy_heatmap_obj)
    popViewport()
    pushViewport(viewport(x = 0.52, y = 0.92, width = 0.777, height = 0.07))
   ...
   pushViewport(viewport(x = unit(1, "cm"), y = unit(2, "cm"), 
      ...
      if (all_accuracy == "false_positive_true_positive_wrong_call") {
        ...
        true_pos_neg_false_pos_neg_wrong_legend()
      }
    popViewport()
dev.off()
...
#    popViewport()
#    pushViewport(viewport(x = 0.52, y = 0.2248, width = 0.777, height = 0.13))
#      grid.rect()
#    popViewport()
#    pushViewport(viewport(x = 0.52, y = 0.92, width = 0.777, height = 0.07))
#      for ( e in 1:length(chr_data$lab_pos) ) {
...
#png(
#  paste0(plot_dir, "legend_test.png"), 
#  height = 8, 
#  width = 22,
#  res = 300,
#  units = "in"
