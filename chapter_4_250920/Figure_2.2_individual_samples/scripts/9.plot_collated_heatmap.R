...
include_metabric <- as.logical(args[11])
...
if (!file.exists(paste0(Robject_dir, "group_epi_heatmap.Rdata"))) {
  ...
  for (s in 1:length(sample_names)) {
    ...
    epi_heatmap <- readRDS(
      paste0(
      	sample_Robject_dir, "4a.final_epithelial_heatmap.Rdata"
      )
    )
    ...
    epi_meta <- readRDS(
      paste0(
      	sample_Robject_dir, "4b.final_epithelial_metadata_without_normals.Rdata"
      )
    )
    epi_meta <- subset(
      epi_meta, 
