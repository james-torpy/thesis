...
Robject_dir <- paste0(out_path, simulation_number, "/Rdata/")
system(paste0("mkdir -p ", Robject_dir))
plot_dir <- paste0(out_path, simulation_number, "/plots/")
system(paste0("mkdir -p ", plot_dir))
table_dir <- paste0(out_path, simulation_number, "/tables/")
...
print(paste0("Possible CNV multipliers are:"))
paste(CNV_multipliers, collapse = ", ")
...
rm(list=ls())
Robject_dir <- "/share/ScratchGeneral/jamtor/projects/thesis/Figure_2.2_accuracy_vs_coverage/results/cancer_simulation/CID4520N_cancer_sim/1/Rdata/"
...
### still need to add Gaussian noise and values of regions of total loss from = zero value ###
...
if ( !file.exists(paste0(Robject_dir, "/2a.simulated_epithelial_df.Rdata")) | 
  !file.exists(paste0(Robject_dir, "/2b.log_modified_fold_change_df.Rdata")) |
...
#    if (CNV_multiplier == 0) {
#
#      temp_section <- modified_df[CNV_record$start[i]:CNV_record$end[i],]
#      # add Gaussian noise:
#      temp_section <- apply(
#        temp_section, 1, function(x) {
#          return(x+rnorm(length(x), mean = 0, sd = 0.05))
#        }
#      )
#      # round to remove decimals:
#      temp_section <- round(temp_section, 0)
#      # make negative values = 0:
#      temp_section <- apply(temp_section, 1, function(x) {
#        x[x<0] <- 0
#        return(x)
#      })
#
#      modified_df[CNV_record$start[i]:CNV_record$end[i],] <- temp_section
#
#    }
    ...
    # if multiplier = 0, change log_median_modified_fold_change to zero value:
    if (CNV_multiplier == 0) {
      log_median_modified_fold_change <- min(log_original_fold_change)
    }
    ...
    zero_count_value <- min(log_original_fold_change)
    log_modified_fold_change[
      log_modified_fold_change < zero_count_value
    ] <- zero_count_value
    ...
    # if multiplier = 0, change log_median_modified_fold_change_df segment to zero value:
    if (CNV_multiplier == 0) {
      log_modified_fold_change_df$count[CNV_record$start[i]:CNV_record$end[i]] <- 
        min(log_original_fold_change)
    }
  }
  ...
  saveRDS(modified_df, paste0(Robject_dir, "/2a.simulated_epithelial_df.Rdata"))
  saveRDS(
    log_modified_fold_change_df, 
    paste0(Robject_dir, "/2b.log_modified_fold_change_df.Rdata")
  )
  ...
  if (!file.exists(paste0(plot_dir, "2a.log_modified_fold_change_from_median_line_only.pdf"))) {
    ...
    pdf(paste0(plot_dir, 
      "2a.log_modified_fold_change_from_median_line_only.pdf"), width = 20)
      print(p)
    dev.off()
  ...
  # plot counts:
  if (!file.exists(paste0(plot_dir, "2b.log_modified_fold_change_from_median.pdf"))) {
    p <- ggplot(log_modified_fold_change_df, aes(x=number, y=count))
    p <- p + geom_point(colour = "#E8D172")
    p <- p + xlab("Genomic coordinates")
    p <- p + scale_x_continuous(
    ...
    p <- p + scale_y_continuous(
      ...
      labels = c("Zero counts", "-1", "0", "1", "2", "3", "4"),
      limits = c(zero_count_value, max(log_modified_fold_change_df$count))
    )
    for (end in chromosome_ends) {
      p <- p + geom_vline(xintercept=end)
    }
    ...
    pdf(paste0(plot_dir, "2b.log_modified_fold_change_from_median.pdf"), width = 20)
      print(p)
    dev.off()
  }

} else {
  modified_df <- paste0(Robject_dir, "/2a.simulated_epithelial_df.Rdata") 
  log_modified_fold_change_df <- paste0(Robject_dir, "/2b.log_modified_fold_change_df.Rdata")
  final_CNV_record <- paste0(Robject_dir, "/2c.CNV_record.Rdata")
}


################################################################################
### 7. Bind new epithelial and original stromal counts, check against metadata
...
# ensure metadata has same cells as epithelial_df and label stromal cells:
epithelial_metadata <- infercnv_metadata$metadata[colnames(epithelial_df),]
...
if (!file.exists(paste0(no_downsample_dir, "input_matrix.txt"))) {
  ...
  write.table(new_counts, paste0(no_downsample_dir, "input_matrix.txt"), quote=F,
    sep="\t", col.names=T, row.names=T)
...
################################################################################
### 8. Downsample new counts and save ###
################################################################################
