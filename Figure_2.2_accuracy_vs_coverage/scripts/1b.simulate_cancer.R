...
if (!file.exists(paste0(noise_dir, "/noise_df.Rdata"))) {
  ...
  outfiltered_counts <- outfiltered_counts[,1:10000]
...
#  ######
  ...
  # include only genes in TxDb.Hsapiens.UCSC.hg38.knownGene:
#  noise_input <- noise_input[
#    rownames(noise_input) %in% gene_chr$symbol,
#  ]

  #saveRDS(noise_input, paste0(noise_dir, "/noise_input.Rdata"))
  #noise_input <- readRDS(paste0(noise_dir, "/noise_input.Rdata"))

#  # trim noise_input according to genes in new_counts:
#  trimmed_noise_input <- noise_input[
#    rownames(new_counts),
#  ]
#  saveRDS(trimmed_noise_input, 
#    paste0(noise_dir, "/trimmed_noise_input.Rdata")
#  )
...
#    names(ind) <- rownames(input_df)[ind]
...
#  ######
#
#  p1 <- ggplot(input_df, aes(log10(input)))
#  p1 <- p1 + geom_area(stat = "bin", bins = 15)
#  for (g in 1:length(input_end_genes)) {
#    print(input_end_genes[g])
#
#    input_df_rownames <- gsub("-|_", "", rownames(input_df))
...
#  }
#
#  pdf(paste0(noise_dir, "log10_noise_input_smoothplot.pdf"))
#    p1
#  dev.off()
#
#  ######
...
#no_downsample_dir <- paste0(out_dir, "no_downsampling/without_noise/input_files/")
#if (!file.exists(paste0(no_downsample_dir, "input_matrix.txt"))) {
#  system(paste0("mkdir -p ", no_downsample_dir))
#  write.table(nondownsampled_counts_without_noise, paste0(no_downsample_dir, "input_matrix.txt"), quote=F,
#    sep="\t", col.names=T, row.names=T)
...
#    # downsample simulated dataset without noise added:
#    print(paste0("Downsampling counts by ", proportion, "..."))
#    downsampled_counts_without_noise <- downsampleMatrix(
#      downsampled_counts_without_noise, proportion, bycol=T
#    )
#    print("Total counts after downsampling:")
#    print(sum(as.vector(downsampled_counts_without_noise)))

#    # add noise to downsampled counts:
#    downsampled_counts <- downsampled_counts+noise_counts

#    downsample_dir <- paste0(out_dir, proportion, 
#      "_downsampling/without_noise/input_files/")
#    system(paste0("mkdir -p ", downsample_dir))
#    write.table(
#      downsampled_counts_without_noise, paste0(downsample_dir, "input_matrix.txt"), 
...
#    actual_proportion <- 
#      sum(
#        as.vector(downsampled_counts)
#      )/sum(
#        as.vector(new_counts)
#      )
#    print(paste0("Actual proportion of denoising post-noise addition = ",
#      actual_proportion))
