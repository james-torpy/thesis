...
print(paste0("Analysis mode of normal InferCNV run = ", 
  analysis_mode, "_mode"))
neutral_signal_range <- unlist(strsplit(args[3], split = "_"))
...
if (
  !file.exists(
    paste0(Robject_dir, "0a.normal_artefact_record.Rdata")
  ) | 
  !file.exists(
    paste0(Robject_dir, "0b.normal_artefact_genes.Rdata")
  )
) {
  ...
  infercnv_averages <- apply(infercnv_output, 2, mean)
  
  for ( i in 1:(ncol(infercnv_output)-9) ) {
    averages_per_cell <- apply(infercnv_output[i:(i+9)], 1, mean)
    ...
    if ( length(which(averages_per_cell < neutral_signal_range[1])) > 
      ...
      segment_record <- data.frame(
        ...
        average_signal = mean(infercnv_averages[i:(i+9)]),
        type = "loss"
      )
  ...
  chr_data <- fetch_chromosome_boundaries(infercnv_output, ref_dir)
  ...
  artefact_genes <- colnames(infercnv_output)[permutated_artefact]
  # remove duplicates:
  artefact_genes <- artefact_genes[!duplicated(artefact_genes)]
  saveRDS(artefact_genes, paste0(Robject_dir, "artefact_genes.Rdata"))
  filtered_infercnv <- infercnv_output[
    , !(colnames(infercnv_output) %in% artefact_genes)
  ]
  
  print(
    paste0(
      "Dimensions of prefiltered_infercnv = ", 
        paste(as.character(dim(infercnv_output)), collapse=",")
    )
  )
  print(
    paste0(
  ...
  pdf(
    paste0(plot_dir, "filtered_infercnv_output_plot.pdf"), 
    height = 13, width = 20
  )   
...
filtered_df <- merged_df[!(rownames(merged_df) %in% artefact_genes),]
...
################################################################################
### 3. Run InferCNV to check no artefacts are present ###
################################################################################
