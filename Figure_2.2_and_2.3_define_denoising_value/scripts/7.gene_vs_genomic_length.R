...
###################################################################################
### 5. Determine range of values at 50 gene intervals ###
###################################################################################

temp_intervals <- seq(50, 400, 50)

for (v in 1:length(temp_intervals)) {

  int_lengths <- length_data[
    length_data$length > temp_intervals[v]-5 & length_data$length < temp_intervals[v]+5,
  ]

  if (v==1) {
    length_stats <- data.frame(
      gene = temp_intervals[v],
      mean_genomic = round(mean(int_lengths$genomic_length), 1),
      min_genomic = min(int_lengths$genomic_length),
      max_genomic = max(int_lengths$genomic_length)
    )
  } else {
    length_stats <- rbind(
      length_stats,
...
write.table(
  length_stats,
  paste0(table_dir, "length_stats.txt"),
  quote = F,
  sep = "\t",
  col.names = T,
  row.names = F
